  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.user) {
        ...
        // 3. Verificación de Propiedad del Establecimiento (Uso de maybeSingle para evitar rupturas)
        ...
        if (establishment?.id) {
          ...
          // 👇 Si el auth es correcto pero no hay local, le damos paso como SuperAdmin raíz 👇
          console.log("[UPLINK_OK]: Nodo maestro detectado sin establecimiento vinculado de forma directa.");
          onLoginSuccess('SUPERADMIN_ROOT_NODE');
        }
      }
    } catch (err: any) {
      console.error(err);
  ...
  return (
    <div className="min-h-screen bg-aura-bg text-aura-green font-mono flex items-center justify-center p-6 relative overflow-hidden select-none">
      ...
      >
        ...
        {error && (
          <div className="mb-8 border-2 border-aura-red bg-aura-red/10 p-4 text-aura-red flex items-center gap-3 text-xs font-bold uppercase">
        ...
        <div className="mt-12 pt-6 border-t-[1px] border-dashed border-aura-dark flex justify-between items-center text-[10px] opacity-30">
